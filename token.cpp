 ...
 begin:
  ...
  case 0:
    ...
    if (*p == '\''){
      p++;
      *q = p;
	  printf("(%d:%d)sehv: \'\'  tek dirnaq arasinda hecne yoxdur. \n", cur_row, p - last_col);
      return;
    }
    ...
    if ( *p == '\r' ){
	   p++;
       *q = p;
       printf("(%d:%d)sehv: \'  tek dirnaq baglayan dirnaq qoyulmayib. \n", cur_row, p - last_col);
       cur_row++;
       return ;
    }
  ...
  case 8:
    ...
    if ( *p == '\r' || p>= end){
      ...
      *q = p;
      ...
      printf("(%d:%d)sehv: \\%c  yanlish xususi simvol. \n", cur_row, p - last_col, *p);
      p++;
      *q = p;
      return;
...
void DFA_NUMB (char **q){
 ...
 error:
  ...
  memset(s, '\0', 100);
  strncpy(s,tmp,k);
  printf("(%d:%d)sehv: %s duzgun eded deyil \n", cur_row, p - last_col, s);
  err_cnt++;
  *q = p;
  return;
...
int read_src_and_std_files(char **text_ptr, char *name_src, char *name_std) {

	FILE *fp_src, *fp_std;
	long lSize_src, lSize_std, lSize;
	char *buffer_src, *buffer_std;
	char *ptr;

	fopen_s(&fp_src, name_src, "rb");
	...
	lSize_src = ftell(fp_src);
	rewind(fp_src);

	/* allocate memory for entire content */
	//because of some bugs in tokenizer some extra 2 byte space needed
	// at the end of source bufer
	buffer_src = (char *)calloc(1, lSize_src + 1 + 2);
	if (!buffer_src) fclose(fp_src), fputs("yer ayrilmadi", stderr), exit(1);

	/* copy the file into the buffer */
	fread(buffer_src, 1, lSize_src, fp_src);
	//if (fread(buffer_src, 1, lSize_src, fp_src))
	//	fclose(fp_src), free(buffer_src), fputs("menbe fayli oxunma xetasi", stderr), exit(1);
	//add 2 byte space to the end of source buffer 
	*(buffer_src + lSize_src + 0) = '\r';
	*(buffer_src + lSize_src + 1) = '\n';

	///////////////////////////////////////////////////////////////////
	/* reading source to buffer done now read standart funcs file */

	fopen_s(&fp_std, name_std, "rb");
	...
	fseek(fp_std, 0L, SEEK_END);
	lSize_std = ftell(fp_std);
	rewind(fp_std);

	/* allocate memory for entire content */
	buffer_std = (char *)calloc(1, lSize_std + 1);
	if (!buffer_std) fclose(fp_std), fputs("yer ayrilmadi", stderr), exit(1);

	/* copy the file into the buffer */
	fread(buffer_std, 1, lSize_std, fp_std);
	//if (lSize_std != fread(buffer_std, 1, lSize_std, fp_std))
	//	fclose(fp_std), free(buffer_std), fputs("standart kitabxana faylini oxunma xetasi", stderr), exit(1);

	lSize = lSize_src + lSize_std + 2;
	// additional 2 bytes for \n and \r btw src & std
	// and 2 add bytes for ending \0\0
	*text_ptr = (char *)malloc(lSize);
	memset(*text_ptr, '\0', lSize);
	...
	//read src to text
	strncpy(ptr, buffer_src, lSize_src);
	ptr += lSize_src;
	//put gap btw
	*ptr = '\r';
	ptr++;
	*ptr = '\n';
	ptr++;
	//read std
	strncpy(ptr, buffer_std, lSize_std);
	ptr += lSize_std;
	*ptr = '\0';
	ptr++;
	*ptr = '\0';

	//printf("src and std %s", *text_ptr);
...
void get_tokens(char *name){
  ...
  char *p, *q, tmp[1024];
  int fd,k, size, size_src, size_std, nmb;

  /*
  std::ifstream in_src(name);
  std::string contents_src((std::istreambuf_iterator<char>(in_src)), std::istreambuf_iterator<char>());
  size_src = contents_src.length();

  std::ifstream in_std("standart.esr");
  std::string contents_std((std::istreambuf_iterator<char>(in_std)), std::istreambuf_iterator<char>());
  size_std = contents_std.length();

  size = size_src + size_std;
  text = (char *)malloc(size);
  memset(text, '\0', size);

  p = text;
  strcpy(p, contents_src.c_str());

  p += size_src;
  strcpy(p, contents_std.c_str());
...
//  exit(0);
  ...
  size = read_src_and_std_files(&text, name, "standart.esr");
...
*/
  ...
  text = (char *)malloc(size + size_std + 10);
  memset(text, '\0', size + size_std + 10);
...
case HAL1S:
  ...
  case FARGS_IDT_VAL:
    printf("FARGS_IDT_VAL");
    break;

  case FARGS_CRG1_VAL:
    printf("FARGS_CRG1_VAL");
    break;

  case FARGS_CRG2_VAL:
    printf("FARGS_CRG2_VAL");
    break;

  case FARGS_STRMEM_VAL:
    printf("FARGS_STRMEM_VAL");
    break;

  case FARGS_EXPR_VAL:
    printf("FARGS_EXPR_VAL");
    break;
...
case  EGER_YOXSA_OPER:
    printf("EGER_YOXSA_OPER");
    break;	

case  NEQEDERKI_OPER:
    printf("NEQEDERKI_OPER");
    break;	
...
case  ARYBRK_VAL:
	sprintf(msg, "ARYBRK_VAL");
    break;

case  BRK_SHERT:
    sprintf(msg,"BRK_SHERT");
    break;
