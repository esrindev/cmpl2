...
extern void sehvleri_yoxla_yeni(struct tok_list *tree);
...
//new rule not used in follow table construction
int cons_print_int_tok(struct tok_list_el *ptr, struct tok_list_el **tll_ref,  int *pos){
  ...
  if (matchid(a,PRINT_INT) && matchid(b,NOQTEV)){
...
int cons_vdec_tok(struct tok_list_el *ptr, struct tok_list_el **tll_ref,  int *pos){
  ...
  if ((matchid(a,TAM) || matchid(a,KESR) || matchid(a,SIMVOL)) && ( match_ia(b) || matchid(b, SEQ))){
		ptr->id  = VDEC;
		ptr->childs[0] = tll;
		ptr->childs[1] = tll->next;
		ptr->cld_cnt   = 2;
...
int cons_plus_tok(struct tok_list_el *ptr, struct tok_list_el **tll_ref,  int *pos){
 ...
 if (tll!=NULL)
  ...
  if ((match_inacf(a) || matchid(a, EXPR) || matchid(a, BRK_VAL)) && \
	   match_lp_opr(b) && \
	  (match_inacf(c) || matchid(c, EXPR) || matchid(c, BRK_VAL) )){
		ptr->id  = EXPR;
       ptr->childs[0] = tll;
       ptr->childs[1] = tll->next;
       ptr->childs[2] = tll->next->next;
...
//r48 used together with cons_plus_tok as one rule
//r48   
...
int build_cur_layer_dax_cap (struct tok_list *tree){
  ...
  pos = 0;      /* how many elements scanned so far  */
   ...
   while (tll!=NULL){
    ...
    if (cons_print_int_tok(ptr, &tll, &pos))
      continue;
...
int parser_core(struct tok_list *tree){
  ...
  // now we are ready to check grammar errors
