...
void set_val(unsigned char *adrs, long double val, int tip){
...
int exec_daxilet_idt(struct band_list_el *bll){
  ...
  if (vstr == NULL){
    printf("cannot read from input bufer.\n");
    return ;
  }
...
int exec_put_ret_addr( struct band_list_el *bll){
    ...
    push_to_stek(&glbstk, (void *)(bll->head_pos));
...
int exec_qaytar1(struct band_list_el *bll){
  ...
  pos =(int*)pop_from_stek(&glbstk);
  /* obyekt stekini de nezere al  */

  /* we return 0, only at qaytar, davamet and dayan opeators  */
    return pos;
}
