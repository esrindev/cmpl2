...
int cons_kod_part(struct tok_list_el *ptr, struct tok_list_el **tll_ref,  int *pos){
  ...
  if ( (sim_com_blk(a) || matchid(a, KOD_PART)) && (sim_com_blk(b) || matchid(b, KOD_PART))){
       ptr->id = KOD_PART;
       ptr->childs[0] = tll;
       ptr->childs[1] = tll->next; 
       ptr->cld_cnt = 2;
...
int cons_eger_yoxsa_opr(struct tok_list_el *ptr, struct tok_list_el **tll_ref,  int *pos){
...
int cons_hal2_tok(struct tok_list_el *ptr, struct tok_list_el **tll_ref,   int *pos){
  ...
  if (((matchid(a,HAL1) || matchid(a, HAL1S) || matchid(a, SUS1)) && \
       (matchid(b,KOD_PART) || sim_com_blk(b))  && (matchid(c, CLSBLK) || matchid(c, HAL2))) || \
...
int build_cur_layer_koddata (struct tok_list *tree){
  ...
  while (tll!=NULL){
    ...
    if (cons_eger_yoxsa_opr(ptr, &tll,   &pos))
      continue; 
